...
6. **知识库里的所有具体数值（阈值、法规数字、罚款金额、MADL/NSRL、ppm 限值、税率）必须附带原始 URL，且 URL 要指向给出该数值的官方页面（不是首页）。如果无法确认具体 URL，宁可不写数字，改为"请以官方来源为准"**——错误的权威数字比没有数字更危险。详见 `.claude/agents/knowledge-curator.md` 第 0 条规则
